...
export function ensureCloudinaryConfigured(): void {
  ...
  /** process.env a veces no refleja .env.local en el proceso del Route Handler; combinedEnv sí. */
  const pick = (key: string) =>
    process.env[key]?.trim() || combinedEnv[key]?.trim();

  const cloudName = pick("CLOUDINARY_CLOUD_NAME");
  const apiKey = pick("CLOUDINARY_API_KEY");
  const apiSecret = pick("CLOUDINARY_API_SECRET");
  const rawUrl = pick("CLOUDINARY_URL");
